...
import model.*;
import io.*;
public class Mascara extends HttpServlet {
	...
	for(int i = 0; i < items.size(); i++){
		if( uri.equals("/shop/Mascara/add" + items.get(i).getArt()) ) {
			if(session != null){
				try{
			...
			response.sendRedirect("/shop/Mascara");   
			return;   
		}
	}	
	...
	public String getMainPage(HttpServletRequest request, HttpServletResponse response){
		...
		sb.append("<section class=\"courses\">");
       		...
       		for(int i = 0; i < items.size(); i++){
			if(!items.get(i).getSale().equals("0") && items.get(i).getCat().equals("Mascara") && (!items.get(i).getAmount().equals("0"))){
				sb.append("<article>");
				sb.append("<figure>");
				...
				if(session != null){
					sb.append("<form method=\"GET\" action=\"/shop/Mascara/add" + items.get(i).getArt() + "\">\n");
