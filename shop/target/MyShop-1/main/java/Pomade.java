...
import model.*;
import io.*;
public class Pomade extends HttpServlet {
	...
	for(int i = 0; i < items.size(); i++){
		if( uri.equals("/shop/Pomade/add" + items.get(i).getArt()) ) {
			if(session != null){
				try{
			...
			response.sendRedirect("/shop/Pomade");   
			return;   
		}
	}	
	...
	public String getMainPage(HttpServletRequest request, HttpServletResponse response){
		...
		sb.append("<section class=\"courses\">");
       		...
       		for(int i = 0; i < items.size(); i++){
			if(!items.get(i).getSale().equals("0") && items.get(i).getCat().equals("Pomade") && (!items.get(i).getAmount().equals("0"))){
				sb.append("<article>");
				sb.append("<figure>");
				...
				if(session != null){
					sb.append("<form method=\"GET\" action=\"/shop/Pomade/add" + items.get(i).getArt() + "\">\n");
