...
public class Sale extends HttpServlet {
	...
	for(int i = 0; i < items.size(); i++){
		if( uri.equals("/shop/Sale/add" + items.get(i).getArt()) ) {
			if(session != null){
				try{
			...
			response.sendRedirect("/shop/Sale");   
			return;   
		}
	}	
	...
	new XmlItem().loadItemsFromFile("basket.xml", item);
	if( uri.equals("/shop/Sale")){
		out.println(getSalePage(request, response));
	}
	...
	public String getSalePage(HttpServletRequest request, HttpServletResponse response){
		HttpSession session = request.getSession(false);
		items = new ArrayList<Item>();
		new XmlIO().loadItemsFromFile("shop.xml", items);
		int p = 0;
		...
		sb.append("<section class=\"courses\">");
       		...
       		for(int i = 0; i < items.size(); i++){
			if(!items.get(i).getSale().equals("0") && (!items.get(i).getAmount().equals("0"))){
				sb.append("<article>");
				sb.append("<figure>");
				...
				if(session != null){
					sb.append("<form method=\"GET\" action=\"/shop/Sale/add" + items.get(i).getArt() + "\">\n");
